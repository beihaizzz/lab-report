...
- **准确优先于效率**：宁可多做一步验证，不可跳步。不确定时停下来确认，而不是"大概是对的"就继续。违反流程比慢更糟糕。
...
| `/lab -upgrade` | 从 GitHub 拉取最新版本更新本地 skill |
...
## /lab -upgrade — 更新 Skill 版本

从 GitHub（https://github.com/beihaizzz/labmate.skill）拉取最新版本更新本地 skill。

执行逻辑：

1. 检测当前运行的平台（Claude Code / OpenCode / Cursor / 其他）

2. 根据平台定位本地 skill 目录：
   - Claude Code: ~/.claude/skills/lab/
   - OpenCode: 读取 ~/.config/opencode/opencode.json 中的 skill 路径
   - Cursor: .cursor/rules/ 或 ~/.claude/skills/lab/
   - 其他: 请用户提供 skill 目录的绝对路径

3. git -C skill目录 pull origin main

4. 确认更新后的版本号（读取 SKILL.md 中 **Current version** 行）
   展示 git log --oneline -3

5. 告知用户更新结果：旧版本 → 新版本，新增了什么
...
生成反馈报告前，AI 必须逐项检查并记录结果。
任何一项未通过，在报告开头汇总列出：

| 检查项 | 验证方式 | 违规标记 |
|--------|---------|----------|
| 图片全部 look_at 分析？ | 对比图片文件总数 vs look_at 实际调用次数 | IMAGE_SKIPPED |
| inspect_template 已执行？ | .labmate/template-inspect.json 是否存在 | INSPECT_MISSING |
| 合并单元格特殊处理？ | 检查 fill 过程中是否有 merge-cell 警告 | MERGE_UNHANDLED |
| label cell 未被覆盖？ | validate_docx.py --inspect 输出 | LABEL_OVERWRITTEN |
| 报告迭代了几轮？ | git log 中 fill/fix 相关提交次数 | ITERATIONS=N |
| 有未确认就修改的内容？ | 对比用户原始请求 vs AI 实际修改范围 | UNCONFIRMED_CHANGE |
| 有凭经验猜测的数据？ | 在内容中搜索"大概""应该""估计""约" | GUESSED_DATA |

归因追溯链——每个问题用缩进链展示因果：
截图未 look_at
  → image-map.json 为 placeholder 模式
    → 配图注释靠 AI 猜测
      → 用户手动改了 4 处注释
...
4. 生成反馈文件到 .labmate/feedback/ 目录（旧项目 .lab-report/feedback/）：
   - feedback_report_vN.md  — 可读报告
   - feedback_report_vN.json — 结构化数据
